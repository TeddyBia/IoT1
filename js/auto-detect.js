...
function runAuto(Traw, Hraw) {
  ...
  // Servo mở theo mức nóng (tuyến tính 0..180)
  let servoTarget = Math.round((yFan / 100) * 180);

  // 5) Slew-rate limit để mượt
  yFan = Math.round(slew(yFanPrev, yFan, SLEW_MAX));
  yLed = Math.round(slew(yLedPrev, yLed, SLEW_MAX));
  const servo = Math.round(slew(servoPrev, servoTarget, Math.round((SLEW_MAX/100)*180)));
